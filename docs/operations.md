...
- timeline restore는 event replay를 사용하고, snapshot이 있으면 snapshot 이후 이벤트만 replay한다.
...
### Projection Worker 설계

현재 구현은 제출 범위 안에서 정합성을 우선해 동기 projection을 사용한다. 비동기화할 경우에는 event append transaction 안에서 outbox row까지 같이 저장하고, worker가 outbox를 읽어 projection과 snapshot을 갱신한다.
...
worker
  ...
  -> apply projection by event_id
  -> create snapshot when sequence % N == 0
  -> mark processed
```

idempotency:

- projection 처리 이력은 `event_id` unique key로 관리한다.
- worker가 같은 outbox를 두 번 읽어도 projection은 한 번만 반영한다.
- snapshot은 `(session_id, server_sequence)` unique key로 중복 생성을 막는다.

retry / DLQ:

- 일시 오류는 지수 백오프와 jitter로 재시도한다.
- 같은 outbox가 정해진 횟수를 넘기면 DLQ로 이동한다.
- DLQ에는 `event_id`, `session_id`, 실패 원인, 마지막 stack trace hash, retry count를 남긴다.
